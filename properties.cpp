...
_inline int hamming_weight (int x)
{
    int res;
    for (res = 0; x > 0; x = x >> 1)
        res = res + (x & 0x01);
...
int get_delta_uniformity (float *robustness)
{
	...
	for (i = 0; i < m_shift; i++)
	{
		for (j = 0 ; j < n_shift; j++)
		{
			int x = 0;
			for (x = 0; x < m_shift; ++x) {
				if ((input_array[x] ^ input_array[x ^ i]) == j) {
					//DDT[i ^ j][input_array[i] ^ input_array[j]]++;
					DDT[i][j]++;
				}
			}
	...
	for (i = 0; i < m_shift; i++)
	{
		...
		{
			//if (DDT [i][j] > delta && (i != 0 && j != 0))
			if (DDT[i][j] > delta && (i != 0))
				delta = DDT [i][j];	
		}
	}

	for (i = 1; i < m_shift; i++)//这里改成了m
	{
		if (DDT [i][0] != 0)
			R++;
	}
...
int nonlinearity (int **wt, uint *components)
{
	...
	for (j = 0; j < columns; j++)
	{
		if (wt[0][j] < 0)//?应写为temp = abs(wt[0][j]);更好？
			temp = wt[0][j] * (-1);
		for (i = 1; i < rows; i++)
		{
			if (abs(wt[i][j]) > temp)
		...
		if (components[j] < min)
			min = components[j];
	}//使用的是nxj论文中的式2.20. 其中wt[i][j]是S(j, i) 先算出内层的max， 然后用2^n - max, 然后除2，由于负号的原因，此时的外层max变成了min
	return min;
}
...
int propagation_characteristics () //the higher the better
{
	...
	while (order <= N) {
		for (int i = 1; i < m_shift; ++i) {
			if (hamming[i] == order + 1) {
				get_derivative(input_array, help, i, m_shift);
				//int q = 0;
				//uint* tem = (uint*)malloc(m_shift * sizeof(int));
				
				//for (q = 0; q < m_shift; ++q) {
					//int p;
					//for (p = 0; p < m_shift; ++p)
						//tem[p] = help[q][p];
					if (!isbal(help, m_shift))return order;
				//}
			}
		}
		order++;
	...
	do 
	{
		for (i = 1; i < m_shift; i++)
		{
			...
			if (order == hamming[i])//如果order等于hamming[i]
			{	//α为i时候的f(x) + f(x + α)值
				get_derivative(input_array, help, i, m_shift);
				//计算出了所有的f(x) + f(x + i),放入help
				//could be done with tt, wt, balance
				int bal = m_shift / n_shift;//如果是平衡函数，那么每个结果应该出现几次
				for (j = 0; j < n_shift; j++)//这里改成n?  表示对于每一个输出，统计出现的次数
				{
					for (z = 0; z < m_shift; z++)//遍历所有输入，计算j出现的次数
					{
						if (help[z] == j)//由于f(x) + f(x + i)是平衡函数，所以对每一个j应该只有一个z使其相等
							count++;
					}
					if (count != bal)
						return order - 1;
...
{
    int  tmp, weight, deg;
	uint i, j, rows, columns, max = 0xffffffff;//这里改了，本来是0
	...
	for (j = 0; j < columns; j++)
	{
		if (anf[rows - 1][j] != 0)//x1x2..xm的系数
			deg = M;
		else
		{
			for (deg = 0, i = 1; i < (rows - 1); ++i)
				if (anf[i][j] != 0) //anf[i][j]表示输出按照j线性组合时，x相乘项可以表示成i的那一项系数是否为1？
				{
					for (weight = 0, tmp = i; tmp > 0; tmp >>= 1)//计算出i的汉明重量？
						weight = weight + tmp%2;
					if (weight > deg)
						deg = weight;
				}
		}
		components[j] = deg;//components里放的应该是输出按照j线性组合形成的布尔函数的代数次数
		//printf("%d\n", components[j]);
		if (components[j] < max)//应该取min?
			max = components[j];
	}
	return max;
}
...
{ 
	...
	for (keyi = 0; keyi < keySize; keyi++)//2^n + 2^(n - 1) + 
	{
		...
		{		
			...
			confusionCharacteristic[coefficientCounter]=(float)coefficientSum / (float)(inputSize << 2);//这里本来没有这个左移 
			coefficientCounter ++;
	...
	for (i = 0; i < coefficientSize; i++)
		var = (float) (var + (float) pow((confusionCharacteristic[i] - (float) mean), 2));//方差
